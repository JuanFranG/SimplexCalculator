...
function validarPrimerosValores() {
  ...
  var cantidadRestriccionesInput = document.getElementById('cantidadRestricciones');
  ...
  if (cantidadVariablesInput.value === "" || cantidadRestriccionesInput.value === "") {
    editElement.textContent = "Los campos no pueden estar vacíos";
    return;
  }

  if (cantidadVariables < 2) {
    editElement.textContent = "La cantidad de variables debe ser mayor o igual a 2";
    return;
  }

  if (cantidadRestricciones < 1) {
    editElement.textContent = "La cantidad de restricciones debe ser mayor o igual a 1";
    return;
  }

  if (!Number.isInteger(cantidadVariables)) {
    editElement.textContent = "La cantidad de variables debe ser un número entero";
    return;
  }

  if (!Number.isInteger(cantidadRestricciones)) {
    editElement.textContent = "La cantidad de restricciones debe ser un número entero";
    return;
  }
...
function obtenerPrimerosValores() {
  ...
  cantidadRestricciones = document.getElementById("cantidadRestricciones").value;
...
function validarEntradas() {
  ...
  var inputrestricciones = document.getElementsByClassName('inputrestricciones');
  ...
  for (var i = 0; i < datosFuncion.length; i++) {
    ...
    if (valor === "") {
      editElement.textContent = "La casilla " + (i + 1) + " de la función no puede estar vacía.";
      return false;
    }
    
    if (!datoRegex.test(valor)) {
      editElement.textContent = "La casilla " + (i + 1) + " de la función tiene un formato incorrecto. Por favor, ingresa un número válido.";
      return false;
    }
  }
  ...
  for (var i = 0; i < inputrestricciones.length; i++) {
    ...
    if (valor === "") {
            editElement.textContent = "La casilla " + (i + 1) + " de las restricciones no puede estar vacía.";
      return false;
    }
    
    if (!datoRegex.test(valor)) {
      editElement.textContent = "La casilla " + (i + 1) + " de las restricciones tiene un formato incorrecto. Por favor, ingresa un número válido.";
      return false;
    }
  }
  ...
  for (var i = 0; i < inputs.length; i++) {
    ...
    if (valor !== '') {
      if (valor.includes('/')) {
        ...
        valor = Number.isInteger(valor) ? valor.toString() : math.fraction(valor).toString();
      }
...
function obtenerDatosRestricciones() {
  ...
  for (var i = 0; i < restriccionesDivs.length; i++) {
    ...
    var resultadoInput = restriccionDiv.querySelector('input[type="text"].inputrestricciones:last-of-type');
    ...
    for (var j = 0; j < inputs.length - 1; j++) {
      ...
      if (coeficiente !== "") {
        if (coeficiente.includes('/')) {
          ...
          coeficiente = Number.isInteger(coeficiente) ? coeficiente.toString() : math.fraction(coeficiente).toString();
        }
      }
    ...
    if (resultadoInput.value !== "") {
      ...
      if (resultado.includes('/')) {
        ...
        resultado = Number.isInteger(resultado) ? resultado.toString() : math.fraction(resultado).toString();
      }
...
function borrarElementos() {
  var divs = ['vectorC', 'vectorX', 'vectorCb', 'vectorXb', 'vectorB', 'matriz-container', 'vectorO', 'vectorR'];
