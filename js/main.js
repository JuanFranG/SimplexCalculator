...
function ejecucion() {
   ...
   restriccionesDesigualdad = restriccionesVanilla;
    ...
    [restriccionesIgualdad, variablesArtificiales] = convertir_desigualdades_en_igualdades(restriccionesDesigualdad);
    funcionObjetivoNueva = agregar_variables_func_obj(func_objetivo, variablesArtificiales);
    ...
    ({vectorXb, vectorCb} = crearVectoresMatrizB(vectorX, vectorCnum, columnasIdentidad));
    console.log('-------------------------------------------------------------------------------------------------- ');
  ...
  function iterar(){
    ...
    if(minimoR!=0){
        ...
        var niteracion = document.getElementById('niteracion');
          niteracion.textContent = "Iteracion ["+n+"]"

        if(n!=1){
          ({matrizB,vectorXb,vectorCbnum}=reemplazarColumnas(matrizB,posicionMinimoO,matrizAnum,posicionMinimoR,vectorXb,vectorCbnum,vectorX,vectorCnum))
          }
        matrizBnuminv = math.inv(matrizB);
        resultado1 = math.multiply(math.matrix(vectorCbnum), math.matrix(matrizBnuminv));
        resultado2 = math.multiply(resultado1, math.matrix(matrizAnum));
        resultado3 = math.round(math.subtract(resultado2, math.matrix(vectorCnum)),3);
        //posicion devalor minimo de r
        ({valorNegativoEncontrado, minimoR, posicionMinimoR} = menorValorPosicionR(resultado3));
        // b-1*A
        resultado4 = math.round(math.multiply(math.matrix(matrizBnuminv), math.matrix(matrizAnum)),3);
        //b-1*B
        resultado5 = math.round(math.multiply(math.matrix(matrizBnuminv), math.matrix(vectorBnum)),3);
        //Z
        ...
        console.log("vector R: ",math.format(resultado3), " --menor valor[",minimoR,"] posicion[", posicionMinimoR,"]")
        console.log("Matriz b-1*A: ",resultado4)
        console.log("Vector b-1*B:  ",math.format(resultado5))
        console.log("Z: ",resultado6)
        console.log("vector O:  ",math.format(resultado7)," --menor valor[",minimoO,"] posicion[", posicionMinimoO,"]")
        ...
        if(puntoDegenerado(resultado5.toArray())==true){
          ...
          poliedroabierto.textContent = "Se ha detectado un punto degenerado en el problema. Esto puede afectar la convergencia del método"
        }
        ...
        if(poliedroAbierto(resultado7.toArray())==true){
          ...
          poliedroabierto.textContent = "Se ha detectado un poliedro abierto en el problema. Esto implica que no existe una solución óptima finita, y el proceso del método simplex no puede converger"
        ...
        if(minimoR===0 && error===false){
          ...
          if (objetivoFuncion == "Minimización") {
            ...
            msgresultado.textContent = "La solucion optima es Z= "+(-1*resultado6) 
            msgresultado2.textContent = mostrarSolucionesVariables(vectorXb,resultado5.toArray())
          }
          else{
          ...
          document.getElementById("noacotado").style.display = 'block';
          var poliedroabierto = document.getElementById('noacotado');
          poliedroabierto.textContent = "El problema tiene solución ilimitada (no acotada). Una variable debe entrar a la base pero ninguna variable puede salir. "
          document.getElementById("iterar").style.display = 'none';
          document.getElementById("solucionDirecta").style.display = 'none';
          document.getElementById("reset").style.display = 'block';
        }
